...
export function useStudioGenerationFlow(args: UseStudioGenerationFlowArgs) {
  ...
  const resetAutoGeneratedProjectGate = useCallback(() => {
    if (project && autoGeneratedProjectIdRef.current === project.id) {
      autoGeneratedProjectIdRef.current = null;
    }
  }, [project]);
      ...
      fireAndForget(
        ...
        (error) => {
          ...
          setStatusLine(
            getAsyncActionErrorMessage(
              error,
              "Studio could not start the first automatic generation.",
            ),
          );
        },
      );
